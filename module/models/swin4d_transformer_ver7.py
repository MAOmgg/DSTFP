...
class PatchMergingV2(nn.Module):
    ...
    def forward(self, x):
        x_shape = x.size()#16 10 10 10 20 24
        b, d, h, w, t, c = x_shape
        x = torch.cat(
...
class SwinTransformer4D(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # ====== 新增重建头部 ======
        self.recon_head = self.build_reconstruction_head(
            in_channels=self.num_features,
            out_channels=1,
            upsample_factor=2 ** (len(depths))  # 根据编码器深度计算上采样倍数
        )
    def build_reconstruction_head(self, in_channels, out_channels, upsample_factor):
        """构建重建头部"""
        layers = []
        ...
        # 最终输出层
        layers.extend([
            nn.Conv3d(current_channels, 64, kernel_size=3, padding=1),
            nn.ReLU(inplace=True),
    ...
    def forward(self, x):
        ...
        # outputs = []
        # for t in range(deepest_feature.shape[-1]):
        #     frame = deepest_feature[..., t]  # [B, C, D_low, H_low, W_low]
        #
        #     # 空间上采样重建
        #     reconstructed_frame = self.recon_head(frame)  # [B, out_chans, D, H, W]
        #     outputs.append(reconstructed_frame)
        return x #4 192 2 2 2 20 (b, c, h, w, d, t)
